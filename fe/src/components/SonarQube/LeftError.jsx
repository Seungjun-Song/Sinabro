...
const LeftError = ({ item, isSelect, setIsSelect, index, isDark, isSelected, onSelectKey }) => {
  ...
  return (
    <motion.div>
      <div style={{ color: "gray", padding: "1rem 0.5rem", fontSize: "0.9rem" }}>
        .../{getLastNthPathSeparatorSubstring(item.component, 3)}
      </div>
      <motion.div
        whileHover={{ backgroundColor: isDark ? "#ccd5f893" : "#ccd5f893", border: "1px solid transparent" }}
        ...
        style={{
          ...
          transition: "0.3s",
          border: index === isSelect.id ? "1px solid transparent" : "1px solid #ccd5f8",
          position: "relative",
        }}
      >
        {item.message}
        <AnimatePresence>
          {(isHover || index === isSelect.id) && (
            <motion.div
              ...
              style={{ position: "absolute", top: 0, left: 0, width: "5px", backgroundColor: "#7B87D9" }}
            />
          )}
        </AnimatePresence>
        <motion.div
        ...
        >
          <FontAwesomeIcon size="lg" icon={isSelected ? faSquareCheck : faSquare} />
        </motion.div>
      </motion.div>
    </motion.div>
  );
